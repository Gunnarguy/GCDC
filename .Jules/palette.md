...
## 2025-06-18 - Native Streamlit Selection Limits for Better UX
**Learning:** For components like `st.multiselect` where a maximum number of choices is desired (e.g., forming a team of 4), using the native `max_selections` argument prevents invalid choices before they occur and disables the dropdown cleanly when the limit is reached. This is a significantly better user experience than allowing users to select more items and then retroactively slicing their list and showing an `st.warning`.
**Action:** Always prefer native UI constraint features (like `max_selections=N`) over implementing manual validation and post-action warning messages.
