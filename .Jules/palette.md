...
## 2025-06-25 - Use max_selections parameter in st.multiselect for better UX
**Learning:** In Streamlit, manually warning users and truncating their selection in `st.multiselect` provides a clunky, reactive user experience. Streamlit provides a native `max_selections` parameter that preemptively disables further selections once the limit is reached, offering a much smoother and more intuitive interaction model without needing reactive warning boxes.
**Action:** Always prefer the native `max_selections=N` parameter in `st.multiselect` over implementing manual limit validation and truncation logic.
