...
## 2026-07-22 - Native Streamlit Component Constraints
**Learning:** Manual backend validation combined with generic warning messages (e.g., `st.warning`) for input constraints creates a disjointed UX. By using native Streamlit component arguments like `max_selections` for `st.multiselect`, the frontend instantly prevents invalid interactions, saving users from frustrating trial-and-error.
**Action:** Prefer using native component constraints (e.g., `max_selections`, `max_chars`, `min_value`/`max_value`) to enforce limits directly on the client side, rather than implementing custom Python-side validation checks and warning messages.
