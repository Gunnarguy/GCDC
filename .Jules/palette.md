...
## 2025-06-25 - Using Streamlit Native Max Selections
**Learning:** Manually checking selection limits and displaying warnings creates unnecessary friction and reactive UI updates. Streamlit's `st.multiselect` has a native `max_selections` parameter that prevents the user from selecting more than the allowed amount in the first place, providing a much better, preventative user experience.
**Action:** Always use the native `max_selections=N` parameter in `st.multiselect` instead of implementing manual validation and warning logic.
