...
## 2025-07-18 - Native Selection Limits in Streamlit Multiselect
**Learning:** Manual validation logic for limiting selections in a multiselect component (e.g., checking `len(selections) > N` and showing an `st.warning`) results in poor UX because it lets users perform invalid actions before correcting them. Streamlit provides a native `max_selections` parameter that automatically disables the input when the limit is reached, offering immediate, preventative feedback. Additionally, adding a `placeholder` enhances input clarity.
**Action:** Use native `max_selections` and `placeholder` parameters in Streamlit `st.multiselect` instead of implementing manual validation and warning messages.
