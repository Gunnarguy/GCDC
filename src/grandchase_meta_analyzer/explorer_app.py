...
def render_pills(label: str, items: list[str]) -> None:
    ...
    st.markdown(f"<b>{html.escape(str(label))}</b><br>{pills}", unsafe_allow_html=True)
...
def render_meta_database(data: dict[str, pd.DataFrame]) -> None:
    ...
    with tabs[0]:
        if unit_df.empty:
            ...
            attr_pick = fc1.multiselect(
                "Attribute",
                attrs,
                placeholder="Choose attribute...",
                help="Filter units by their elemental attribute (e.g., Fire, Water).",
            )
            class_pick = fc2.multiselect(
                "Class",
                classes,
                placeholder="Choose class...",
                help="Filter units by their class (e.g., Assault, Ranger).",
            )
            job_pick = fc3.multiselect(
                "Job Type",
                job_types,
                placeholder="Choose job type...",
                help="Filter units by their job type (e.g., Base, T).",
            )
            ...
            if not display.empty:
                pick = st.selectbox(
                    ...
                    display["name"].tolist(),
                    index=None,
                    placeholder="Select a unit to view details...",
                    help="Select a unit from the table above to view their recommended traits, gear, and build details.",
                    key="unit_inspect",
                )
                if pick:
    ...
    with tabs[1]:
        if builds_df.empty:
            ...
            build_hero = st.selectbox(
                ...
                help="Select a hero to view their specific content builds (e.g., PvE, PvP Defense).",
                key="build_hero",
            )
            if build_hero:
    ...
    with tabs[4]:
        if teams_df.empty:
            ...
            content_pick = st.selectbox(
                ...
                help="Select a content mode (e.g., Altar of Time, Vulcanus) to view recommended teams.",
                key="ct_content",
            )
            if content_pick:
